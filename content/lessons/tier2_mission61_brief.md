# Tier 1, Mission X — The Badge of Hassle

## Core Targets
- **Linguistic Pattern:** The Gossip Opener (`frame:youknow-la`) and The Adverb Machine (`frame:adverb-aa`)
- **Register:** delight
- **Dramatic Ingredient:** turn — the scene flips on a reveal partway through
...
## Scenario Context
Maya and Raj dedicate this episode to unpacking the deep cultural weight of a single concept: *அலைச்சல்* (the running-around / hassle). Raj opens by complaining about the sheer exhaustion of managing an upcoming family event, leaning heavily into family gossip mode to set the scene. Partway through, Maya flips the script with a delightful reveal: in a Tamil extended family, complaining about *அலைச்சல்* is actually a humblebrag. If you aren't burdened with the hassle, it means you aren't considered important enough to the family. The exhaustion is a badge of honor.

## Word Payload

**NEW (2 words):**
- **அலைச்சல்** (*alaichal*) — The hassle / the running-around
- **frame:youknow-la** (*...-la*) — The gossip opener (நம்ம {person} இருக்கான்ல… → 'you know our {person}, right…')

**CALLBACKS (3 words):**
- **மாமா** (*maama*) — Uncle (maternal) [floor-gap]
- **frame:adverb-aa** (*-aa*) — {X}+ஆ → 'X-ly / as X' — நல்லா, மெதுவா, தண்ணியா, இன்ஜினியரா இருக்கேன்: one machine turns anything into an adverb [struggled]
- **frame:happened-aachu** (*-aachu*) — {V}+ச்சு / ஆச்சு → 'it happened / it's become' [struggled]
...
## Notes
The Trip Sprint (Phase 1.5) is ACTIVE. The payload aggressively targets the Trip Deck's EAR-ONLY gossip components (catch items) and struggled frames. Since the episode form is `lore`, the entire episode centers on Maya and Raj end-to-end, serving as a deep dive without a standard Intercept scene.
