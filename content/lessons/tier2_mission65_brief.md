# Tier 2, Mission 65 — The Apology Call

## Core Targets
- **Linguistic Pattern:** The Polite Ask (`frame:polite-nga`)
- **Register:** reconciliation
- **Dramatic Ingredient:** stakes
- **Scenario Shape:** dispute
- **Location class:** extended_family
- **Energy:** low
- **Episode Form:** phone_call

## Scenario Context
A tense, low-energy phone call with an estranged elder brother (அண்ணா) or maternal uncle (மாமா) to clear the air after a past dispute. The stakes are real—a damaged family relationship hangs in the balance. This dynamic naturally forces a shift into the respectful register, demanding "The Polite Ask" (`-nga`) as the caller seeks forgiveness, attempts to schedule a real conversation ("when will you be free?"), and asks for a little time to explain ("give me a minute"). 

## Word Payload

**NEW (5 items):**
- **frame:polite-nga** — {verb}+ங்க → the polite ask — சொல்லுங்க, குடுங்க, உக்காருங்க, வாங்க [NEXT ENGINE — teach first (show it), NEVER cold-quiz]
- **அப்புறமா பேசலாமா?** (*appurama pesalaama?*) — Shall we talk later?
- **நான் அப்புறமா கூப்பிடுறேன்** (*naan appurama kooppiduren*) — I'll call later
- **எனக்கு போன் பண்ணு** (*enakku phone pannu*) — Call me
- **எப்ப ஃப்ரீ-யா இருப்ப?** (*eppa free-ya iruppa?*) — When will you be free?

**CALLBACKS & DECK TARGETS (8 items):**
- **frame:giveme-noun** — {noun} குடுங்க → give me {noun} [hinted→cold deck — force this]
- **frame:if-ndhaa** — {verb}+ந்தா / இல்லன்னா → 'if…' [hinted→cold deck — force this]
- **புரியல** (*puriyala*) — I don't understand [hinted→cold deck — force this]
- **அப்படியா?!** (*appadiya*) — Oh really?! [cold-pending deck]
- **பிடிக்காது** (*pidikkaadhu*) — I don't like [due callback / floor-gap]
- **சீக்கிரம்** (*seekkiram*) — Quickly [due callback / floor-gap]
- **அழகான** (*azhagana*) — Beautiful [due callback / floor-gap]
- **சொன்னாங்க** (*sonnaanga*) — They said [due callback / floor-gap]

## Vocabulary Fence (the sea — build from these)
175 known words. The Architect should build dialogue from this pool. Words outside this list must be answerable from context within seconds.
...
## Notes
- **Trip Sprint Phase 1.5 is active.** Keep driving the `Trip Deck` cold. The learner defaults to `venum` — heavily scaffold `குடுங்க` in natural requests until it supersedes the fallback.
- **`frame:polite-nga`** is a novel engine. **Teach it first** (demonstrate and gloss it) before expecting the learner to deploy it on a cold quiz.
- Ensure the remaining **FLOOR-GAP TARGETS** (`சரி`, `ஆனா`, `இருக்கு`, `அண்ணா`, `கிட்ட`, `மாமா`) from the ticket are actively woven into the dialogue to support forced recognition.
- Keep the dialogue aligned with the **`-nga`** register default; even when adapting the "phone_scheduling" items, pivot them to `-nga` for safety and accuracy given the "extended_family" recipient.
