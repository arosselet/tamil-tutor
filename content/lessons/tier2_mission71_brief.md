```markdown
# Tier 2, Mission 71 — The Idli Blockade Confession

## Core Targets
- **Linguistic Pattern:** The Obligation-Command Collision (I must go vs. Sit and eat)
- **Register:** embarrassment
- **Dramatic Ingredient:** genre — a confession
- **Scenario Shape:** debrief
- **Location class:** home_social
- **Energy:** medium
- **Episode Form:** phone_call

## Scenario Context
The protagonist is hiding in the bathroom, whispering on a phone call to a friend. They are confessing their embarrassment: they are running late for a train and desperately need to leave (`station-kku poganum`, `avasaram irukku`), but their well-meaning mother-in-law has blockaded the door with more food, insisting they sit and eat one more bite (`ukkarunga`, `saapidunga`). Every time they try to excuse themselves, they don't understand (`puriyala`) the rapid-fire Tamil guilt-trip and end up awkwardly sitting back down. The tension comes from trying to politely escape without causing a scene.
...
## Notes (optional)
Calibration: Andrew's `-nga` vs `-laamaa` choice is cold both ways; now we strain `avasaram irukku` and `poganum` at speed to clear the stragglers, colliding directly with the `-nga` command form. The `phone_call` form allows low-stakes production in English while the 'catch' items (`quote-nu`) drift in via recounted background dialogue, building on the 07-22 hearsay catch.
```
