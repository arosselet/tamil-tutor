# Tier 2, Mission 60 — The Weight of "If"

## Core Targets
- **Linguistic Pattern:** The Conditional "If" and the Collective "We" (`frame:if-ndhaa` and `frame:we-om`)
- **Register:** grief/nostalgia
- **Dramatic Ingredient:** turn
- **Scenario Shape:** pattern_riff
- **Location class:** extended_family
- **Energy:** low
- **Episode Form:** lore

## Scenario Context
Raj and Maya do a deep-dive into the cultural and emotional weight of the conditional "if" (`frame:if-ndhaa`) and the phrase "கடைசில பாக்கலாம்" (we'll see at the end). They trace these frames to the chaotic tea-table negotiations of family travel and life planning. Initially, Maya playfully frames these phrases as standard stall tactics used to avoid the immense hassle (அலைச்சல்) of making plans. However, the scene flips on a reveal (the turn): Raj shares a story fueled by nostalgia and grief about his late father (அப்பா). He explains that for the older generation, saying "if possible" (முடிஞ்சா) wasn't a dismissal. When money was tight and nothing was guaranteed, "if" was a gentle way of holding onto hope, and the collective "we" (`frame:we-om`) was the only certainty they had to lean on.

## Word Payload

**NEW (5 words/phrases):**
- **முடிஞ்சா** (*mudinja*) — If possible / if it's finished (soak order target)
- **கடைசில பாக்கலாம்** (*kadaisila paakalam*) — We'll see at the end (soak order target)
- **அலைச்சல்** (*alaichal*) — The hassle / the running-around (Trip Deck EAR-ONLY)
- **அப்பா** (*appa*) — Father
- **அம்மா** (*amma*) — Mother
...
## Notes (optional)
- **Calibration:** Andrew is in Phase 1.5 (Trip Sprint) and ~30 days from touchdown. His immediate goal is production in social/family settings, prioritizing the Trip Deck. "mudinja" was missed twice recently in chat; this serves as its fresh-context ambush. 
- **Lore Format Constraint:** Because the Episode Form is `lore`, Raj and Maya will carry the episode end-to-end. There is no traditional Intercept scene with Hosts A & B. Raj and Maya act as the narrators and protagonists, delving deeply into the resonance of the payload items.
- **Register Rules:** The standard baseline applies (`-nga/neenga` for everyone unless explicitly noted otherwise). All unknown words (the +1 items outside the fence) must be fully answerable by context within seconds.
