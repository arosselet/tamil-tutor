...
- `curriculum/` — `word_pool.json` (suggestion list of words; Anna picks from it) plus optional curated sets (e.g. `trip_deck.json`) loaded via `sync_state.py seed-deck` — static fields only; every rung is earned.
...
- **LLM prompts embedded in the Python.** Two files hold all of it: `scripts/mandates.py` — the fourteen mandates and addenda (`OUTREACH`, `BASE`, `JUDGE`, `THREAD`, `SLIP`, `REACH`, `MESSAGE`, `CATCH_JUDGE`, `VOICE`, `DRILL`, `LINT`, `SOAK`, and two force-addenda) — and `scripts/run_studio.py`, whose `DIRECTOR` / `ARCHITECT` / `PRODUCER` stay separate because the studio's canon is its own. They state Tamil-specific rules in prose: script vs. phonetic, Woven Thanglish, and worked examples that ARE Tamil morphology — the `-ōm` ending, the honorific `-nga`, `புரியல` as a creditable repair. A port **rewrites** these; there is no seam that makes it cheaper, and pretending otherwise hides the real cost of a fork. This is the port surface a swap-the-`.md`-files pass will miss.
  **Corrected 2026-09-04, and it had been wrong for a month.** This line named `morning_knock.py`, `knock_reply.py` and `render_drill.py` — the files those prompts *used* to live in. `morning_knock`'s went to `mandates.py` on 2026-08-01 when it hit its line ceiling; `knock_reply`'s six followed on 2026-08-24. All three now carry `from mandates import …` and no prompt prose at all, so a porter following this list opened three files and found nothing to rewrite. `s93` now asserts this line against the tree, in both directions.
  **What the guards cover, and the half they cannot.** `s91`'s `SCRIPT_OWNERS` machine-checks everything written in Tamil **script**, with a per-file ceiling: `mandates.py` (11 mechanism lines), `run_studio.py` (2), `sync_state.py` (6). It is structurally blind to the rest — `-ōm`, `-nga`, "Woven Thanglish" and the script-vs-phonetic rule are facts about Tamil written in **Roman letters**, and no script sweep will ever find them. That is why this prose list still has to exist, and why `s93` guards the file NAMES rather than pretending to guard the rules. Note also that a long prompt constant is not automatically port surface: `render_drill.COMMISSION_BRIEF` and `render_soak.FOCUS_BRIEF` are prompt prose and entirely language-agnostic, which is why the guard keys on the `*_MANDATE` convention instead of on "a big string".
...
**Layer 3 — the learner pack (re-instantiate, don't copy).** `protocol/user.md` is this layer's file: the standing facts about the learner that no generator may contradict (rewrite it for the new learner at bootstrap; `voice_canon()` ships it to every voice lane). The heist, the Oracle, field missions, the masks, and the knock cadence are not decoration — they are the **fuel for the momentum system**, tuned to one learner archetype (married into the language, native speaker at home). A different learner supplies their own equivalents rather than deleting them: a **stake** (a reveal, a trip, an exam — the thing mastery climaxes into), an **informant policy** (who the native resource is, and what they must never become — an examiner), and an **outreach contract** (rails and a social contract that fit their life).
