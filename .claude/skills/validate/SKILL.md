...
| `learner.json` has fields `learner`, `last_debrief`, `soak_order`, `recent_audio`, `status` | `sync_state.py` → `write_thin_learner` | `python -c "import json; d=json.load(open('progress/learner.json')); print([f for f in ['learner','last_debrief','soak_order','recent_audio','status'] if f not in d] or 'ok')"` |
