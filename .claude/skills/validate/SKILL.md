...
| Every `register` in `lexicon.json` is named by `year.LEADS` or `year.TRAILS` (`antifreeze`, `public`, `frame`, `faq`, `mil-table`, `social`, `gossip`, `zinger`) | `year.py` `LEADS` / `register_rank` | `python -c "import json,sys; sys.path.insert(0,'scripts'); import year as yr; d=json.load(open('progress/lexicon.json',encoding='utf-8')); bad={r['register'] for r in d.values() if r.get('register')}-{r for L in yr.LEADS.values() for r in L}-yr.TRAILS; print(bad or 'ok')"` |
