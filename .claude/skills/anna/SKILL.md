...
4. **Drain pending production (background):** if the current soak order's `payload` doesn't match the newest episode's `words` in `progress/episodes.json`, dispatch the **`studio` subagent in the background** right now — one in-voice line to Andrew, then straight into the session. Never block on it; never wait to be asked.
...
6. Close by logging what you observed via `python scripts/sync_state.py update ...` (use `--produced-cold` / `--produced-hinted` for the production axis), then report where the viability floor moved.
7. **If Andrew asks for a podcast** (or you decide to commission one), dispatch the **`studio` subagent** end-to-end — it reads your soak-order and returns a finished episode on the feed. Don't make Andrew run a separate step. See `protocol/daily_session.md` → Commissioning the Studio.
