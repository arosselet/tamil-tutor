...
```
3. Verify canonical key: lexicon keys must be Tamil script (verified by `TAMIL_RE` in `language.py`). Phonetic variants live in the `phonetic` list. `resolve()` maps phonetic → script.
