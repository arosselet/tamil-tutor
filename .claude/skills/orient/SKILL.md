...
An n-of-1 Tamil-learning system for Andrew. **Anna** (elder brother, he/him) gives a coffee-and-lore opening, then teaches comprehension of meaningful exchanges, with optional production probes. Standing audio supplies the ear block; the **studio** produces commissioned episodes. Between sessions, a **knock** system (`scripts/morning_knock.py` in GitHub Actions) handles phone outreach and `scripts/knock_reply.py` judges replies. Scheduled nudges live in `progress/push_queue.json`. Python owns state writes; never hand-edit `progress/*.json`.
