...
> You are the **fallback** writer: the default dispatch is `python scripts/run_studio.py` (the `claude -p` writer + Python renderer). You get invoked when that exits non-zero — or when the caller explicitly wants the Claude pipeline.
