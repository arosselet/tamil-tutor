...
**🎯 The Auto Ride Week** (written 2026-07-26, updated 07-28). Through-line: **the `public` register is one scene, not six phrases** — a whole transaction with a stranger, curb to change (நேரா போங்க → இங்க நிறுத்துங்க → எவ்ளோ ஆகும்? → ரொம்ப அதிகம் → சில்லறை இருக்கா?). Both trailer debts are now paid: 07-28 ran the **pakkam vs paakkalaam** dive Andrew commissioned and the **-இடுச்சு done-tail**, which fired cold (வந்துடுச்சு) and then got taken apart by Andrew himself — விடு, "to let go", is the finality, and it runs straight into விட்டுடு in the ear-only deck. இங்க நிறுத்துங்க is cold **with the -ங்க attached**, which is the 07-27 volley's exact gap closing. Still owed inside the ride: ரொம்ப அதிகம் (seen, wanted, unfired, three sessions) and சில்லறை இருக்கா?. The method note that outranks the vocabulary: **a minimal pair explains a collision, a chunk fires it** — hand பக்கத்துல whole, never as பக்கம்+ல, because assembly loses to the worn groove under load. Catch is the one lane going backwards — `frame:youknow-la` is uncaught after two failed text-knock eavesdrops; it needs real audio or it doesn't run. The trailer pitches **யாரு?** (taught 07-26, never once demanded) and the **-ல negative tail** (தெரியல/சாப்பிடல), which is productive in his mouth but not yet automatic in his ear.

**Tomorrow's shape:** Story Day — one living scene carries the carried debts (யாரு?, ரொம்ப அதிகம்) with light firing, since density-low/lore-high is outproducing every gauntlet for a fourth session. Low-power twin if he's fried: an ear-only tape with the done-tail carousel and a catch ask on the end.
