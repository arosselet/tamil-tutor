...
**Where it stands 08-01:** the antifreeze kit is essentially **closed** — the -ங்க ending fires from scratch now (the 07-27 diagnosis was "handed it he reads it back; from scratch it evaporates"), and survival sits 23/34. Delight is still 1 cold of 27. The week's second half is the half that hasn't run.

**Tomorrow's shape:** Story Day — one dinner-table scene carrying the three lines taught 07-31 (கை ருசி, நீங்க சாப்பிட்டீங்களா?, the -ஓம் tail), with the cold demand aimed at the **commissioned-but-untested** slips: `inniki enna panninga?` answered in a room he hasn't seen, which closes or re-opens 1pl-past-om *and* past-tense in one move. Also due, unaided: **என்ன சொன்னீங்க?** (hinted 08-01, doesn't count yet). And `frame:youknow-la` gets collected **live, at the door** — two tapes are on the feed unanswered (07-28 aunties, 08-01 Chitra akka) and re-firing that channel is not working. Low-power twin if he's fried: the தெரியல tape, ear only.
