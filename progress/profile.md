...
**The disease has a sharper name now: A DRAWER OF READY-MADE NO'S (updated 2026-08-29; supersedes the 08-25 'parts held as wholes' reading, which was right but too general).** `chunk-not-machine` is 5× over 19d and on 08-29 it fired twice in ten minutes on the same target — *puriyala*, then *appadi illa*, both for *theriyala*, and *appadi illa* is the identical substitute from 08-25. The shape: he keeps a drawer of **whole, memorised negatives** — each one a real line doing a real job somewhere — and when a negative slot opens his hand grabs whichever is on top, freshest-in-ear first. (I had said *puriyala* four times in ten minutes before demanding *theriyala*. The priming is mine.) The medicine is unchanged and works: **the seat framing** — *appadi illa* is not wrong, its seat is correcting a wrong picture about him (*"Andrew-kku kaaram pidikaadhu"* → *"appadi illa!"*) — then **build, don't grab**: take the verb the question just used and drop the tail on it. Carve, then ambush; never more reps.

**And the ear has its own disease, named 2026-08-29: `tail-inaudible` — the machines-heard bottleneck, finally stated properly.** A four-line eavesdrop came back with **every tailed line inverted** (*varala* → "we're going", *theriyala* → "I know", *kekkala* → "she called and said so") and **the one line carrying no tail word-perfect**. He is not missing those verbs; he fires them. The meaning-bearing tail is **unstressed, and it lands on his ear as silence** — so a sentence arrives as its own mirror image, which is worse than not understanding it. This generalises past `-la` to every machine he owns by mouth and not by ear (`-idum`, `-aachu`, `-ittu`), and it is why 21/26 fire cold while 2/26 are heard. It also moved inside one session: carve the tail live (*puriya* + *illa* → *puriyala* — the `illa` hiding inside a word he has fired for a month, so it is a **factory**, not two memorised words), run minimal pairs where the tail is the only thing moving, then re-check cold — *saapdala* caught clean an hour after it was silence. **That recipe is the dose for every remaining machine.**
