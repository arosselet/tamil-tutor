...
# Google Cloud TTS Voice mapping — Multi-language Chirp 3 HD
# Format: "SPEAKER_KEY": ("language-code", "voice-name")
POLYGLOT_VOICES = {
    "TAMIL_HOST": ("ta-IN", "ta-IN-Chirp3-HD-Achernar"),
    "TAMIL_GUEST": ("ta-IN", "ta-IN-Chirp3-HD-Charon"),
    "FRENCH_HOST": ("fr-CA", "fr-CA-Chirp3-HD-Achernar"),
    ...
    "ENGLISH_GUEST": ("en-US", "en-US-Chirp3-HD-Fenrir"),
}
...
# Regex: matches "**Host:** text", "**Tamil_Host:** text", etc.
SPEAKER_RE = re.compile(
    r"^\s*(?:\*\s*)?\*\*\s*([A-Za-z_]+)\s*:\s*(?:\*\*\s*)?(.*)", re.IGNORECASE
)
...
async def generate_segment_google(text: str, speaker: str, index: int, temp_dir: str, voice_type: str) -> str:
    ...
    # Select the voice and lang from POLYGLOT_VOICES
    lang_code, voice_name = POLYGLOT_VOICES.get(speaker, POLYGLOT_VOICES["TAMIL_HOST"])
