...
- **Audio** — any kind, any time, Anna's judgment: `protocol/commissioning.md` owns authority; `protocol/audio_channels.md` guides capacity and format.
...
- **Escalation** — when the MACHINERY is failing him, file it rather than routing around it: `sync_state.py feedback "…"` banks it, `docs/feature_inbox.md` parks the itch in one line, and the session gets one plain sentence — *"that's a build problem, not a you problem."* A workaround invented in prose is a defect nobody will find.
