...
4. **Caption sheet** (captioned soak, 2026-07-13) — from the FINAL post-dialect script, write `content/captions/<episode>.md`: one blockquote per spoken line — **bold speaker + Tamil** · *phonetic* · English gloss, `<br>`-separated; keep `[SFX]`/`[Pause]` as italic position cues; open with the two-line how-to ("passes 1–2: listen with this open; pass 3+: put it away — blind is the win"). The feed rebuild links it into the episode's show notes automatically. Captions are a companion, never a gate — a failed sheet must not block the episode.
