...
**L0 `state_io.py`** — paths, load/save, `local_today`, the canonical script regexes (the PORT SURFACE: `TAMIL_RE` for "is there script here", `TAMIL_RUN` for "where are the spans" — one range, declared once, guarded by `s70`), token→key `resolve`, the soak payload resolvers, and the read-only predicates `is_unseen` / `soak_pending`. Imports nothing from `scripts/`; everything else may import it.
