...
- **Mistakes are the curriculum.** Being unconfidently wrong out loud is the point, not a cost of admission. A wrong answer is free — grades only ever move up, so a guess can never lose you ground — and it buys the one thing a right answer cannot: a named pattern in the **slip ledger**. Errors accumulate across chat, phone and podcast; twice is a pattern; a pattern earns audio built specifically for it; and it closes only by firing right, unaided, later — never by being corrected. Nothing is thrown away: a closed pattern that comes back returns with its whole history, and the ones that stayed closed are the record of what became muscle memory.
